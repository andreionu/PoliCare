...
import { LayoutDashboard, Calendar, Users, LogOut, Menu, X, Clock } from "lucide-react"
...
const navItems = [
  { title: "Dashboard",    mobileLabel: "Acasă",       href: "/doctor/dashboard",    icon: LayoutDashboard },
  { title: "Programări",   mobileLabel: "Programări",  href: "/doctor/appointments", icon: Calendar },
  { title: "Pacienții Mei",mobileLabel: "Pacienți",    href: "/doctor/patients",     icon: Users },
  { title: "Program",      mobileLabel: "Program",     href: "/doctor/schedule",     icon: Clock },
]

export function DoctorLayout({ children, userName }: DoctorLayoutProps) {
      ...
      <aside className="hidden lg:flex w-64 flex-col border-r bg-card sticky top-0 h-screen shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto py-6">
          <nav className="space-y-1.5 px-4">
            <p className="px-3 mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/50">Portal Medic</p>
            {navItems.map((item) => {
        ...
        <div className="border-t p-4">
          <div className="flex items-center gap-3 mb-3 px-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Medic</p>
            </div>
          </div>
          <Button
            variant="ghost"
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={cn(
                    ...
                    isActive ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white" : "text-muted-foreground hover:bg-blue-50 hover:text-blue-600"
                  )}>
